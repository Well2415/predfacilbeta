...
import { useRelatorioResponsabilidadesData, Responsabilidade } from '../../../../../hooks/relatorioResponsabilidadesData';
import { ViewResponsabilidadeModal } from '../../../Modais/Relatorios/PlanoManutencao/ViewResponsabilidadeModal';

export function Responsabilidades() {
    const hook = useRelatorioResponsabilidadesData();
    const [showPerPage, setShowPerPage] = useState(false);
    const [selectedItem, setSelectedItem] = useState<Responsabilidade | null>(null);
                ...
                <div className="w-full bg-white rounded-[4px] flex flex-col gap-[10px]">
                    <div className="w-full h-[56px] px-[20px] py-[16px] bg-[#F9FAFB] rounded-[4px] border border-[#F0F0F0] flex items-center gap-[10px]">
                        ...
                        <input
                            ...
                            placeholder="Filtrar por Cargo, Função ou Descrição"
            ...
            <div className="w-full px-[40px] flex flex-col justify-center items-center gap-[32px]">
                ...
                <div className="w-full flex flex-col gap-[32px]">
                    ...
                    <div className="w-full px-[20px] flex justify-between items-center">
                        <div className="w-[400px] text-[#3B4141] text-[18px] font-semibold font-['DM_Sans']">Cargo / Função</div>
                    ...
                    <div className="w-full flex flex-col gap-[16px]">
                        {hook.loading ? (
                            ...
                            <div className="w-full py-10 text-center text-[#848484] font-['DM_Sans'] border border-[#F0F0F0] rounded-[4px]">Nenhuma responsabilidade encontrada.</div>
                        ) : (
                            hook.itens.map((item: Responsabilidade) => (
                                <div key={item.id} className="w-full px-[20px] py-[20px] bg-white rounded-[4px] border border-[#F0F0F0] flex justify-between items-center hover:bg-gray-50 transition-colors group">
                                    <div className="w-[400px] text-[#3B4141] text-[16px] font-normal font-['DM_Sans'] pr-4">{item.cargo}</div>
                                    <div className="flex-1 text-[#3B4141] text-[15px] font-normal font-['DM_Sans'] leading-relaxed whitespace-pre-wrap">{item.descricao}</div>
                                    <div className="px-[24px] flex justify-center items-center gap-[16px]">
                                        <div
